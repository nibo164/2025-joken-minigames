...
<script setup>
  ...
  function initGame() {
    ...
    if(currentDifficulty.value === "mystery"){
      ...
      remainingGoldBalls = totalGoldBalls;
      remainingSilverBalls = totalSilverBalls;
  ...
  function gameLoop() {
    ...
    balls.forEach(ball => {
      ...
      if (!waitingForStart.value && intersects(ball.getBounds(), paddle.getBounds())) {
          ...
          if (!block.destroyed && intersects(ball.getBounds(), block.getBounds())) {
            if (currentDifficulty.value === "mystery" && block.isSplitBlock) {
              ...
              for (let j = 0; j < splitCount; j++) {
                let newColor = "white";
                let newScoreValue = 10;
                const totalRemaining = remainingGoldBalls + remainingSilverBalls + 1;
              ...
              newSplits.forEach(splitBall => newBalls.push(splitBall));
            }

            block.destroyed = true;
            score.value += ball.scoreValue;
