...
// 連鎖ボーナス
const CHAIN_BONUS = [0, 0, 8, 16, 32, 64, 96, 128, 160, 512];
...
function createEmptyBoard() {
  return Array.from({ length: BOARD_HEIGHT }, () =>
    Array(BOARD_WIDTH).fill(PUYO_COLORS.EMPTY)
  );
}
...
function checkCollision(dx, dy) {
  ...
  if (
    nextP1.x < 0 ||
    nextP1.x >= BOARD_WIDTH ||
    nextP2.x < 0 ||
    nextP2.x >= BOARD_WIDTH
  )
    return true; // 壁
  if (isValidPos(nextP1.x, nextP1.y) && board.value[nextP1.y][nextP1.x] !== 0)
    return true;
  if (isValidPos(nextP2.x, nextP2.y) && board.value[nextP2.y][nextP2.x] !== 0)
    return true;
...
async function handleChain() {
  ...
  while (true) {
    ...
    // --- スコア計算 ---
    const bonus =
      CHAIN_BONUS[chainCount] || CHAIN_BONUS[CHAIN_BONUS.length - 1];
    const puyoBonus = 0;
  ...
  for (let y = 0; y < BOARD_HEIGHT; y++) {
    for (let x = 0; x < BOARD_WIDTH; x++) {
      ...
      if (connectedGroup.length >= 4) {
        connectedGroup.forEach((puyo) =>
          puyosToClearSet.add(`${puyo.x},${puyo.y}`)
        );
      }
    }
  }
  ...
  if (difficulty.value === "easy") {
    ...
    speedUpInterval = setInterval(() => {
      if (currentFallInterval.value > 200) {
        ...
        gameInterval = setInterval(moveDown, currentFallInterval.value);
        console.log(
          `(Easy) 速度アップ！ 次の間隔: ${currentFallInterval.value}ms`
        );
      }
    }, 10000); // 10秒ごとに実行
    ...
    speedUpInterval = setInterval(() => {
      if (currentFallInterval.value > 100) {
        ...
        gameInterval = setInterval(moveDown, currentFallInterval.value);
        console.log(
          `(Hard) 速度アップ！ 次の間隔: ${currentFallInterval.value}ms`
        );
      }
    }, 1000); // 1秒ごとに実行
  }
...
<template>
  ...
  <div class="game-container">
    ...
    <div class="button-group">
      <button
        @click="startGame"
        :disabled="gameState !== 0 || difficulty === null"
      >
        Start Game
      </button>
